...
ShaderProgram_Colored2D::ShaderProgram_Colored2D(const Shader* _vertexShader, const Shader* _fragmentShader)
	: ShaderProgram("ShaderProgram.2D.Colored", _vertexShader, _fragmentShader)
{
}
...
void ShaderProgram_Colored2D::Render(const Configuration* _config, const Mesh* _mesh, const std::vector<GLfloat>* _colors, const glm::mat4* _transformation) const
{
	...
	expect(color_location != -1, "Failed to get color Position");
